...
function MenuEditor(idSelector, options) {
    ...
    $(document).on('click', '.btnRemove1', function (e) {
        e.preventDefault();
    ...
    function TButtonGroup() {
        ...
        $divbtn.append($btnUp).append($btnDown).append($btnIn).append($btnOut).append($btnRemv);
        return $divbtn;
    }
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var $li = $('<li>').addClass('list-group-item').attr('data-value',v.mid);
            $li.data(itemObject);
